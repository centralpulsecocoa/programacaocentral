...
Deno.serve(async (req) => {
  try {
    ...
    console.log('Starting daily update email job...');
    ...
    const tomorrowSchedulings = schedulings.filter(s => s.date === tomorrowStr);
    ...
    // Gerar HTML do email (simplificado e inline) - isUpdate = true
    const emailHTML = generateEmailHTML(tomorrowSchedulings, groupedSchedulings, true);
    ...
    for (const email of recipientArray) {
      try {
        await base44.asServiceRole.integrations.Core.SendEmail({
          to: email,
          subject: `🔄 Atualização - Programação de Descargas - ${tomorrow.toLocaleDateString('pt-BR')}`,
          body: emailHTML
        });
        sentCount++;
      } catch (error) {
    ...
    console.error('Error in sendDailyUpdateEmail:', error);
    return Response.json({ 
      error: error.message 
    }, { status: 500 });
  }
